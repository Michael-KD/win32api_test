...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        printf("%s usage: shellcodeinjection.exe <pid>", e);
        return EXIT_FAILURE;
    }
    ...
    if (hProcess == NULL) {
        printf("%s failed to get handle to process (%ld), error: %ld", e, PID, GetLastError());
        return EXIT_FAILURE;
    }

    printf("%s got handle to process (%ld)\n", k, PID);
    
    printf("%s allocating %zu-bytes to process memory\n", i, shellcodeSize);
    ...
    printf("%s closing handles to process and thread\n", i);
    CloseHandle(hThread);
    CloseHandle(hProcess);
    printf("%s finished! goodbye!", i);
